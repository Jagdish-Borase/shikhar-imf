$(document).ready(function () {
  ...
  $(document).on('click', '.team-card', function() {
    ...
    $('#tmPhoto').off('error').on('error', function() {
      $(this).hide();
      $('#tmInitials').show();
    });
  ...
  $(document).on('click', '#tmShare', function() {
    ...
    if (navigator.share) {
      ...
      navigator.clipboard.writeText(text).then(function() {
        ...
        setTimeout(function() { $('#tmShare i').removeClass('fa-check').addClass('fa-share-alt'); }, 2000);
      }.bind(this));
    }
  });
  ...
  $('#loginForm').on('submit', function(e) {
    ...
    if (!emailVal) {
      $emailGroup.find('.error-msg').text('Please enter your email or mobile number.').addClass('show');
      $email.addClass('error');
      valid = false;
    } else {
      $emailGroup.find('.error-msg').removeClass('show');
      $email.removeClass('error').addClass('success');
    }

    if (!passVal || passVal.length < 6) {
      $passGroup.find('.error-msg').text('Please enter a valid password (min 6 characters).').addClass('show');
      $pass.addClass('error');
      valid = false;
    } else {
      $passGroup.find('.error-msg').removeClass('show');
  ...
  $('#registerForm').on('submit', function(e) {
    ...
    if (pass && confirm && pass !== confirm) {
      $('#regPasswordConfirm').addClass('error');
      $('#regPasswordConfirm').closest('.form-group').find('.error-msg').text('Passwords do not match.').addClass('show');
      valid = false;
    }
  ...
  $('#quoteForm').on('submit', function(e) {
    ...
    if (!$('#quoteTerms').is(':checked')) {
      $(this).find('.terms-error').addClass('show');
      valid = false;
    } else {
      $(this).find('.terms-error').removeClass('show');
    }
    if (!valid) return;
    var $btn = $(this).find('.btn-submit');
    $btn.prop('disabled', true).html('<i class="fas fa-spinner fa-spin"></i> Submitting...');
